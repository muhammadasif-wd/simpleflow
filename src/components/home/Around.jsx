...
const Around = () => {
    return (
        <div className='my-10 text-white mx-auto w-full h-full'>
            <div className='bg-primaryColor h-[38rem] relative'>
                <img className='h-[38rem] w-[50%] ml-0 opacity-30 object-cover bg-center' src={`${images.side_img}`} alt="" />
                <div className='absolute top-10 lg:flex justify-around'>
                    ...
                    <div className='xl:w-1/2 lg:w-2/3 w-11/12 mx-auto'>
                        ...
                        <div className='grid sm:grid-cols-2 grid-cols-1 md:gap-5 md:mt-10'>
                            {around.map(({ name, icon, desc }, index) => (
                                <article key={index}>
                                    <div className="w-fit gap-3 flex items-center">
                                        <Icon className='w-10 h-10 ml-0' icon={icon} />
                                        <Subtitle subtitle={`${name}`} styles={`font-semibold`} />
                                    </div>
                                    <p className='md:text-base text-sm my-3 ml-14'>{desc}</p>
                                </article>
                            ))}
                        </div>
                    </div>
